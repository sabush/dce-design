...
test_designUI <- function(id) {
  ...
  fluidPage(
    ...
    box(
      width = 4, title = "Design Dimensions", status = "primary", solidHeader = T,
    ...
    column(
      ...
      conditionalPanel('output.enter_design == "TRUE"',
        ns = ns,
        box(
          title = "Enter choice sets", width = 12,
...
test_design <- function(input, output, session) {
  ...
  assessDesign <- eventReactive(input$test_design, {
    validate(
      need(!anyNA(values$chset), "The set of choice sets contains NA values"),
    ...
    if (input$dce_effects == "me") {
      ...
      assess_design(levels, choicesets,
        interactions = "all",
        print_detail = F
      )
